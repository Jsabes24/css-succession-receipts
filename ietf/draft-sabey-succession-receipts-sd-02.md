...
docname: draft-sabey-succession-receipts-sd-02
category: info
...
The underlying receipt format composes with adjacent evidence classes,
including pre-execution authorization receipts
{{I-D.schrock-ep-authorization-receipts}}; that composition is defined at
the receipt layer as an OPTIONAL `authorization_binding` claim {{SR-ID}}
...
The underlying receipt's OPTIONAL `authorization_binding` claim {{SR-ID}}
...
future `spec_version` that adds it as an eighth unit. Because the binding
...
6. **Completeness escalation.** If every digest in both commitment
   ...
   being outside the version 0.1 unit set per {{units}}, is not part of
   an SDR and is neither reconstructed nor required here).
...
-02: Updated the composition reference now that the receipt-layer
`authorization_binding` claim is normatively defined {{SR-ID}}, and
stated its relationship to the version 0.1 disclosure-unit set
({{units}} and {{verification}} step 6): the optional binding is outside
...
new `spec_version`. No wire-format change.
