...
docname: draft-sabey-succession-receipts-03
category: info
...
`credentialSubject.authorization_binding` is an OPTIONAL claim that binds
this succession to a pre-execution authorization of the handoff, recorded
under an authorization-receipt format such as
{{I-D.schrock-ep-authorization-receipts}}. It is additive: a receipt that
omits it verifies exactly as one that predates the claim, its presence
does not change `spec_version`, and a verifier that does not implement it
ignores it while still verifying the receipt.

When present, the claim MUST be an object with these string members:

- `caid` — the canonical action identifier of the handoff, byte-identical
  ...
  specified in {{I-D.schrock-canonical-action-identifier}} and the receipt
  that carries it in {{I-D.schrock-ep-authorization-receipts}}.
- `receipt_hash` — exactly 64 lowercase hexadecimal characters, the
  SHA-256 digest over the canonical representation of the authorization
  receipt as its format defines
  ({{I-D.schrock-ep-authorization-receipts}} for the EMILIA Protocol
  ...
  it, embeds it, and hashes the complete representation, in that order.
- `format` — a non-empty authorization-format identifier naming the
  receipt's format, and therefore the canonicalization to apply; for
  example `EP-RECEIPT-v1`, or `EP-QUORUM-v1` for a multi-party
  authorization.

Members inside the claim that a verifier does not recognize are ignored.

A verifier processes the claim as follows. If it does not hold the
...
canonical hash under the canonicalization its `format` defines and require
it to equal `receipt_hash`; then require the authorization receipt's
canonical action identifier, obtained as `format` defines, to equal
`caid`, byte for byte. Both operations are defined entirely by the named
format: for the EMILIA Protocol formats, the canonicalization and the
receipt are specified in {{I-D.schrock-ep-authorization-receipts}} and the
identifier and its derivation in
{{I-D.schrock-canonical-action-identifier}}. A verifier that does not hold
the specification the `format` member names cannot perform this branch and
...
identifier. Any failure invalidates the receipt.

The binding introduces no new failure mode for a receipt that omits it,
...
corpus {{SR-CORPUS}} pins the rule and its evaluation order with a
cross-format vector (corpus revision `ahr-v0.2/r2`) contributed by the
authorization-receipt format's author.
